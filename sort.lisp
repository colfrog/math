...
(defun merge-array (a b start-left end-left start-right end-right)
  (let ((left-index start-left)
    ...
    (when (< left-index end-left)
      (dotimes (i (- end-left left-index))
	(setf (aref b write-index) (aref a (+ left-index i)))
	(setf write-index (1+ write-index))))
    (when (< right-index end-right)
      (dotimes (i (- end-right right-index))
	(setf (aref b write-index) (aref a (+ left-index i)))
	(setf write-index (1+ write-index))))
