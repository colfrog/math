...
(defun array-min-max (a)
  (let ((min nil) (max nil))
    (dotimes (i (length a))
      (when (or (not min) (< (aref a i) min))
	(setf min (aref a i)))
      (when (or (not max) (> (aref a i) max))
	(setf max (aref a i))))
    (cons min max)))

(defun countingsort (a)
  (let* ((s (make-array (length a) :initial-element 0))
	 (minmax (array-min-max a))
	 (l (car minmax))
	 (u (cdr minmax))
	 (d (make-array (1+ (- u l)) :initial-element 0)))
    (dotimes (i (length a))
      (setf (aref d (- (aref a i) l))
	    (1+ (aref d (- (aref a i) l)))))
    (dotimes (j (1+ (- u l)))
      (when (/= j 0)
	(setf (aref d j) (+ (aref d (1- j)) (aref d j)))))
    (dotimes (k (length a))
      (let* ((i (1- (- (length a) k)))
	     (j (- (aref a i) l)))
	(setf (aref d j) (1- (aref d j)))
	(setf (aref s (aref d j)) (aref a i))))
    s))
